...
| `com/slabbed/placement/PlacementCollisionShapes` | Discovers stored collision owners above a queried body without creating chunks or changing unstored geometry. |
| `com/slabbed/placement/StoredCollisionSource` | Supplies the shared query context to optional Lithium collision adapters. |
| `com/slabbed/mixin/LithiumStoredPlacementContextMixin` | Reads the optional Lithium sweeper context from its declaring class. |
| `com/slabbed/mixin/StoredPlacementCollisionMixin` | Applies a stored placement offset once to native collision shapes. |
| `com/slabbed/mixin/StoredPlacementCollisionIteratorMixin` | Adds stored owners missed by the vanilla collision and support search. |
...
| `com/slabbed/mixin/client/*` | The client mixins declared in `slabbed.client.mixins.json`; every member is a render-offset, cull, lighting, texture-connection, remesh or offset-raycast mixin. The optional Sodium hooks preserve exposed height-step faces and shade inset tops at their drawn height. The optional Continuity hook prevents top overlays from crossing height steps. Optional Better Block Entities hooks and Sodium context accessors align static and animated block-entity rendering with stored placement heights. Optional Create Fly container hooks apply the same height to instanced chest/shulker base poses. Non-recursive, so the excluded `mixin/client/recorder/**` cannot creep back under this row. |
