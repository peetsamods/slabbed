...
Use the repo-native schema-3 and schema-6 consumers instead of the installed legacy `$record` summarizers:
...
- `--run-id UUID` selects exactly one discovered schema-3 or schema-6 base/child run by manifest UUID.
...
- `--require-c3-pair-fields` implies `--require-player-pairs` and exits 7 unless the selected
  supported run advertises the complete extended action header/capability and contains at least one
...
schema child, so a derived report cannot overwrite raw recorder evidence.
...
cannot establish authorship and is rejected. Proxy-only evidence is valid but says `NO_PLAYER_EVIDENCE`.

Schema 6 adds raw action rows plus terminal logical-placement attempts. Do not treat a terminal RED as a
gameplay count until the clicked owner and newly placed child have been distinguished. TEST 35 documents
the current chain-model RED and the separate logical-attempt merger caution in
`docs/process/LIVE_RECORDER_ADAPTER.md`.

Full contract: `docs/process/LIVE_RECORDER_ADAPTER.md`.

## Historical C3 candidate closure record
