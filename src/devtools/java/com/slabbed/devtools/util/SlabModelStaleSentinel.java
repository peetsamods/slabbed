...
public final class SlabModelStaleSentinel {
    ...
    public static DiagnosticSeverity diagnosticSeverity(String kind) {
        return switch (kind == null ? "" : kind) {
            case KIND_DIVERGENT, KIND_ABSENT -> DiagnosticSeverity.RED;
            // Pair geometry is a candidate, not a failure verdict. Connector collision bodies may
            // legally cross a cell boundary and the placement law may intentionally preserve
            // different authored offsets. Promote only after independent world-space model,
            // outline, collision, and contact-plane evidence identifies a harmful mismatch.
            case KIND_ENSEMBLE_INTERPENETRATION, KIND_ENSEMBLE_GAP -> DiagnosticSeverity.YELLOW;
