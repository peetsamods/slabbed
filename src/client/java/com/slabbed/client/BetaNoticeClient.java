 ...
 *
 * <p>Ported from the 1.21.11 sibling. ONE API-shape adaptation was required: this branch pins
 * Minecraft 1.21.1 (yarn {@code 1.21.1+build.3}), which predates Mojang's {@code ClickEvent}
 * refactor into a sealed-interface-of-records shape (that refactor landed later, and is what
 * the 1.21.11 source's {@code new ClickEvent.RunCommand(...)} depends on). Verified directly
 * against this branch's actual pinned mappings (grepped
 * {@code <home>/.gradle/caches/fabric-loom/1.21.1/net.fabricmc.yarn.1_21_1.1.21.1+build.3-v2/mappings.tiny}):
 * {@code net/minecraft/text/ClickEvent} is a plain class with constructor
 * {@code (ClickEvent.Action, String)} and a nested {@code ClickEvent.Action} enum containing
 * {@code RUN_COMMAND} — there is no {@code ClickEvent.RunCommand} type on this version. So this
 * file uses {@code new ClickEvent(ClickEvent.Action.RUN_COMMAND, "/command")} instead. Every
 * other API used here ({@code WorldSavePath} package, {@code ClientPlayConnectionEvents.JOIN},
 * {@code ServerInfo.address}) was independently verified to resolve identically on this
 * branch's pinned jar/mappings — no other adaptation was needed.
 */
