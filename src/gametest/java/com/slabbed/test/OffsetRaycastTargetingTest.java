 ...
 * overhaul ported from main's 39a345e7.
 *
 ...
 *
 * <p>Branch note (fix/1211-slab-log-dodo-log): unlike main at the time of the cutover,
 * this branch renders connecting blocks (fence/wall/pane) LOWERED on vanilla slabs
 * (GH #21, 92516668), so the connection-block test here asserts the outline follows the
 * lowered render — the opposite of main's {@code connectionBlockOutlineNotOffset} — and
 * that the nearest-hit raycast targets the lowered fence where vanilla DDA cannot.
 */
public final class OffsetRaycastTargetingTest {
    ...
    // ──────────────────────────────────────────────────────────────────────────
    // 6. CONNECTION BLOCKS FOLLOW THE RENDER (this branch's GH #21 policy): a fence
    //    on a vanilla bottom slab renders LOWERED here, so its outline must be
    //    offset to match, and the nearest-hit raycast must target it at the lowered
    //    mid-height where vanilla DDA cannot. (Main at the cutover asserted the
    //    opposite — un-lowered fences — because its model zeroed connection dy;
    //    this branch's model tracks getYOffset since 92516668.)
    // ──────────────────────────────────────────────────────────────────────────
    @GameTest(structure = "fabric-gametest-api-v1:empty")
    public void connectionBlockOutlineMatchesLoweredRender(TestContext ctx) {
        ServerWorld world = ctx.getWorld();
        BlockPos origin = ctx.getAbsolutePos(BlockPos.ORIGIN);
        ...
        ctx.assertTrue(dy == -0.5,
                "fixture invalid: fence on a vanilla bottom slab should lower -0.5 (GH #21), got " + dy);
        ...
        ctx.assertTrue(Math.abs(minY - dy) < 1.0e-6,
                "fence-on-slab outline must be offset to match the lowered render (expected minY="
                        + dy + ", got " + minY + ")");

        // Targeting: horizontal ray through the post column at fence.y - 0.25 (inside
        // the lowered post, in the slab's cell layer, above the slab's own top face).
        double y = fence.getY() - 0.25;
        Vec3d eye = v(origin, 3.5, y - origin.getY(), 0.5);
        Vec3d end = v(origin, 3.5, y - origin.getY(), 7.5);
        BlockHitResult hit = slabbed(world, eye, end);
        ctx.assertTrue(hit.getType() == HitResult.Type.BLOCK && hit.getBlockPos().equals(fence),
                "offset-aware raycast should target the lowered fence " + fence
                        + ", got " + hit.getType() + " " + hit.getBlockPos());

        HitResult van = vanilla(world, eye, end);
        boolean vanillaSawFence = van.getType() == HitResult.Type.BLOCK
                && ((BlockHitResult) van).getBlockPos().equals(fence);
        ctx.assertFalse(vanillaSawFence,
                "control failed: vanilla unexpectedly hit the lowered fence at mid-height");
        ctx.complete();
    }
