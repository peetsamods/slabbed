...
public final class SlabbedLabBeta4LiveShapeGoblinClientGameTest implements FabricClientGameTest {
    ...
    public void runTest(ClientGameTestContext ctx) {
        ...
        System.out.println("[SLABBED_BETA4_LIVE_GOBLIN_START]"
        ...
        try (TestSingleplayerContext singleplayer = ctx.worldBuilder()
            ...
            if (!proveFreshStructure(ctx, singleplayer)) {
                System.out.println("[SLABBED_BETA4_LIVE_GOBLIN_DONE] status=STRUCTURE_FAIL");
                return;
            }
            System.out.println("[SLABBED_BETA4_LIVE_GOBLIN_BASELINE]"
            ...
            System.out.println("[SLABBED_BETA4_LIVE_GOBLIN_HITBOX_" + verdicts.hitbox + "]"
                    + " wrongOwner=" + verdicts.wrongOwner
                    + " miss=" + verdicts.miss
                    + " observedCases=" + verdicts.observedCases);

            String releaseBlockers = releaseBlockers(verdicts);
            System.out.println("[SLABBED_BETA4_LIVE_GOBLIN_SUMMARY]"
                    + " structure=GREEN"
    ...
    private static boolean proveFreshStructure(ClientGameTestContext ctx, TestSingleplayerContext singleplayer) {
        ...
        ctx.runOnClient(mc -> {
            if (mc.world == null) {
                System.out.println("[SLABBED_BETA4_LIVE_GOBLIN_STRUCTURE_FAIL] reason=client_world_missing");
                return;
            }
    ...
    private static String runSideCase(
        ...
        ctx.runOnClient(mc -> {
            if (mc.player == null || mc.world == null || mc.interactionManager == null) {
                throw new RuntimeException("[SLABBED_BETA4_LIVE_GOBLIN_STRUCTURE_FAIL] case="
    ...
    private static String runTopFaceCase(ClientGameTestContext ctx, TestSingleplayerContext singleplayer, Verdicts verdicts) {
        ...
        ctx.runOnClient(mc -> {
            if (mc.player == null || mc.world == null || mc.interactionManager == null) {
                throw new RuntimeException("[SLABBED_BETA4_LIVE_GOBLIN_STRUCTURE_FAIL] case=TOP_FACE reason=client_not_ready");
            }
            mc.gameRenderer.updateCrosshairTarget(0.0f);
            HitResult beforeTarget = mc.crosshairTarget;
            ActionResult action = mc.interactionManager.interactBlock(mc.player, Hand.MAIN_HAND, hit);
        ...
        ctx.runOnClient(mc -> {
            if (mc.world == null) {
                throw new RuntimeException("[SLABBED_BETA4_LIVE_GOBLIN_STRUCTURE_FAIL] case=TOP_FACE reason=client_world_missing_after_click");
            }
            ...
            verdicts.observedCases++;
            System.out.println("[SLABBED_BETA4_LIVE_GOBLIN_TOP_FACE_GHOST_" + verdict[0] + "]"
    ...
    private static String runSupportBreakCase(ClientGameTestContext ctx, TestSingleplayerContext singleplayer, Verdicts verdicts) {
        ...
        ctx.runOnClient(mc -> {
            if (mc.player == null || mc.world == null || mc.interactionManager == null) {
                throw new RuntimeException("[SLABBED_BETA4_LIVE_GOBLIN_SUPPORT_BREAK_RED] reason=client_not_ready_before_side_place");
            }
        ...
        singleplayer.getServer().runOnServer(server -> {
            ...
            System.out.println("[SLABBED_BETA4_LIVE_GOBLIN_SUPPORT_BREAK_GREEN]"
                    + " phase=before_break"
        ...
        ctx.runOnClient(mc -> {
            if (mc.world == null) {
                verdict[0] = "RED";
                System.out.println("[SLABBED_BETA4_LIVE_GOBLIN_SUPPORT_BREAK_RED] reason=client_world_missing_after_break");
                return;
            }
            ...
            verdicts.jump |= jump;
            verdicts.observedCases++;
            System.out.println("[SLABBED_BETA4_LIVE_GOBLIN_SUPPORT_BREAK_" + verdict[0] + "]"
                    + " phase=after_break"
                    + " breakPos=" + TOP_SLAB_A.toShortString()
                    + " jump=" + jump
    ...
    private static boolean emitStructureProof(net.minecraft.world.BlockView world) {
        ...
        String marker = bottomA && bottomB && bridgeA && bridgeB && topA && topB && upper
                ...
                : "[SLABBED_BETA4_LIVE_GOBLIN_STRUCTURE_FAIL]";
