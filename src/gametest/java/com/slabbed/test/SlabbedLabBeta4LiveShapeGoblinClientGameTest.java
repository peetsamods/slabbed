...
public final class SlabbedLabBeta4LiveShapeGoblinClientGameTest implements FabricClientGameTest {
    ...
    private static String compoundVisibleReleaseBlockers(CompoundVisibleSlabLaneResults results) {
        if ("GREEN".equals(results.reload) && "PARTIAL".equals(results.triad)) {
            return "model/manualVisual,MaintainerLiveRetest";
        }
        if ("RED".equals(results.reload)) {
            return "reload,model/manualVisual,MaintainerLiveRetest";
        }
        if ("PENDING".equals(results.reload)) {
            return "reloadHarness,model/manualVisual,MaintainerLiveRetest";
        }
    ...
    private static TriadSurface proveTriadSurface(
        ...
        System.out.println("[MAINTAINER_BETA4_COMPOUND_VISIBLE_SLAB_LANE_MODEL_PENDING]"
                + " caseName=" + name
                + " pos=" + pos.toShortString()
                + " state=" + state
                + " marker=" + markerName
