...
/**
 * Slabbed 1.21.11 dy fingerprint.
 *
 ...
 */
public final class Slabbed12111DyFingerprintTest {
    ...
    private static BlockState tsBottomSlab() {
        Block block = Registries.BLOCK.get(Identifier.of("terrainslabs", "dirt_slab"));
    ...
    private static String src(ServerWorld world, BlockPos abs) {
    ...
    private static void fingerprint(TestContext ctx, BlockPos rel, String name, double expected) {
        ...
        Slabbed.LOGGER.info("SLABBED-FP | {} | dy={} | src={}",
                name, String.format(java.util.Locale.ROOT, "%.3f", dy), src(world, abs));
    ...
    public void fpCeilingFlush(TestContext ctx) {
        ...
        fingerprint(ctx, roots, "ceiling_flush", 0.0);
        ctx.complete();
    }
    ...
    @GameTest(structure = "fabric-gametest-api-v1:empty")
    public void fpTsDirectLower(TestContext ctx) {
        BlockPos pos = new BlockPos(2, 2, 2);
        ctx.setBlockState(pos.down(), tsBottomSlab());
        ctx.setBlockState(pos, Blocks.OAK_LOG);
        fingerprint(ctx, pos, "ts_direct_lower", -0.5);
        ctx.complete();
    }
}
