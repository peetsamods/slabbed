...
public final class SlabbedLabLoweredSidePlacementLiveReproClientGameTest implements FabricClientGameTest {
    ...
    private static void runBeta4LiveScreenshotShapeRedProof(
        ...
        if (redSummary.length() > 0) {
            ...
            System.out.println("[MAINTAINER_BETA4_LIVE_SCREENSHOT_HARNESS_FAIL]"
                    + " classification=RED"
                    + " summary=" + summary.replace('\n', '|'));
            throw new RuntimeException("Maintainer beta4 live screenshot shape RED: " + summary);
        }
    ...
    private static void runCompoundSlabNoLegalLaneCase(
        ...
        ctx.runOnClient(mc -> {
            ...
            ActionResult result = mc.interactionManager.interactBlock(mc.player, Hand.MAIN_HAND, hit);
            System.out.println("[MAINTAINER_BETA4_COMPOUND_SLAB_NO_LEGAL_LANE_GREEN]"
                    + " row=" + rowName
                    + " half=" + halfLabel
                    + " result=" + result
