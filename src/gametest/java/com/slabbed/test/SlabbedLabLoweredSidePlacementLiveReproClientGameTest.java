...
public final class SlabbedLabLoweredSidePlacementLiveReproClientGameTest implements FabricClientGameTest {
    ...
    private static void runBeta4LiveScreenshotShapeRedProof(
        ...
        if (redSummary.length() > 0) {
            ...
            System.out.println("[SLABBED_BETA4_LIVE_SCREENSHOT_HARNESS_FAIL]"
                    + " classification=RED"
                    + " summary=" + summary.replace('\n', '|'));
            throw new RuntimeException("the maintainer beta4 live screenshot shape RED: " + summary);
        }
        ...
        throw new RuntimeException("the maintainer beta4 live screenshot shape unexpectedly went green; do not save as RED proof");
    }
    ...
    private static void runBeta4LiveScreenshotSideSlabRedCase(
        ...
        ctx.runOnClient(mc -> {
            ...
            if (!expectedLegalSideSlab) {
                ...
                System.out.println("[SLABBED_BETA4_LIVE_SCREENSHOT_SIDE_SLAB_RED] " + reason);
                redSummary.append(reason).append('\n');
            } else {
                System.out.println("[SLABBED_BETA4_LIVE_SCREENSHOT_SIDE_SLAB_GREEN]"
                        + " result=" + resultText[0]
                        + " actual=" + describeOwnerFacts(mc.world, expectedSideSlabPos));
            }
        });
    ...
    private static void runCompoundSlabNoLegalLaneCase(
        ...
        ctx.runOnClient(mc -> {
            ...
            ActionResult result = mc.interactionManager.interactBlock(mc.player, Hand.MAIN_HAND, hit);
            System.out.println("[SLABBED_BETA4_COMPOUND_SLAB_NO_LEGAL_LANE_GREEN]"
                    + " row=" + rowName
                    + " half=" + halfLabel
                    + " result=" + result
