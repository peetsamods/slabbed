...
public final class SlabSupport {
    ...
    public static boolean shouldOffset(BlockView world, BlockPos pos, BlockState state) {
        ...
        // the guard above misses them and they fell through to hasSlabInColumn (a lantern placed
        // beneath bridged the downward column-walk to a slab, wrongly lowering them -0.5). Their real
        // dy is computed by ceilingHungDecorationDy in getYOffsetInner.
     ...
     * column. Under a LOWERED non-ceiling support (vanilla slab or full block) it follows that
     * support's lowered dy (a TOP slab adds the +0.5 raised-attach baseline so it sits flush, not
     * 0.5 too low). A Terrain Slabs slab renders FLUSH, so it is never treated as a lowered support
     * (the {@code shouldSkipOffset} guard) — a hanger beneath a flush TS slab stays flush even with
     * a carrier lower in the column (e.g. a lantern placed beneath bridging the downward walk to a
     * slab below). Under a normal TOP slab (directly or via a chain of hangers) it floats +0.5;
     * otherwise flush.
     */
