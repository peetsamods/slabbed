...
public final class Beta35LiveTorchCaptureRecorder {
    ...
    public static void recordFrame(World world, Entity camera, PlayerEntity player, HitResult crosshairTarget, float tickProgress) {
        ...
        if (!startLogged) {
            Slabbed.LOGGER.info(
                    "[SLABBED_BETA35_LIVE_TORCH_CAPTURE_START] enabled={} world={} player={}",
                    true,
        ...
        Slabbed.LOGGER.info(
                "[SLABBED_BETA35_LIVE_TORCH_CAPTURE] markerVersion=2"
        ...
        if (sourceTruthCapture) {
            Slabbed.LOGGER.info(
                    "[SLABBED_BETA35_LIVE_TORCH_CAPTURE_SOURCE_TRUTH]"
    ...
    public static void finishPlacementAttempt(World world, PlacementAttemptSnapshot snapshot, ActionResult result) {
        ...
        Slabbed.LOGGER.info(
                "[SLABBED_BETA35_LIVE_TORCH_PLACEMENT_ATTEMPT]"
        ...
        Slabbed.LOGGER.info(
                "[SLABBED_BETA35_LIVE_TORCH_DUAL_SUMMARY]"
                        + " sequence=" + snapshot.sequence()
                        ...
                        + " beta35ReleaseStatus=PAUSED_SLABBED_LIVE_TRACE_REQUIRED"
    ...
    private static String existingContactLogLine(World world, HitResult crosshairTarget, PlayerEntity player, Entity camera) {
        if (world == null) {
            return "[SLABBED_BETA35_LIVE_TORCH_EXISTING_CONTACT] classification=NO_TORCH_NEAR_TARGET";
        }
        ...
        if (!nearestTorch.found()) {
            return "[SLABBED_BETA35_LIVE_TORCH_EXISTING_CONTACT]"
    ...
    private static void logDualStartIfNeeded(World world, PlayerEntity player) {
        ...
        Slabbed.LOGGER.info("[SLABBED_BETA35_LIVE_TORCH_DUAL_TRACE] enabled=true world={} player={}",
