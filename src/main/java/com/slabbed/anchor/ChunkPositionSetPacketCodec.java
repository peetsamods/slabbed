 ...
 * complete readable attachment bytes, including its one-byte presence prefix. The first
 * current-Fabric overflow is 4,063 entries: 4,062 use 32,499 bytes, while 4,063 use
 * 32,507 bytes, over Fabric's 32,502-byte ceiling.
 *
...
final class ChunkPositionSetPacketCodec {
    ...
    static final StreamCodec<RegistryFriendlyByteBuf, LongOpenHashSet> INSTANCE =
            StreamCodec.of(ChunkPositionSetPacketCodec::encode, ChunkPositionSetPacketCodec::decode);

    private ChunkPositionSetPacketCodec() {
    }

    private static void encode(RegistryFriendlyByteBuf buf, LongOpenHashSet positions) {
    ...
    private static LongOpenHashSet decode(RegistryFriendlyByteBuf buf) {
    ...
    private static LongOpenHashSet decodeLegacy(RegistryFriendlyByteBuf buf, int count) {
