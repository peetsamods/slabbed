...
/**
 * The pure policy for the server's shifted hit-validation center.
 *
 * <p>This does NOT widen Minecraft's distance tolerance. It only says when an already-lowered target
 * may move that validation center by its frozen visible depth, so an aim the client honestly made at a
 * deep visible body is not thrown away by a check that still measures from the grid cell. Target-owned
 * use validates against the target's translated cell even when vanilla needs a held item for the
 * interaction; block placement keeps its resolver-family and owner-envelope gates. Flat owners and
 * out-of-envelope hits fall through to vanilla untouched.
 *
 * <p>PORT NOTE (1.21.1): the donor's {@code BlockState.isSolidRender()} does not exist in Yarn 1.21.1,
 * so every "is this owner a real surface" test here uses the view-independent
 * {@link SlabSupport#isOpaqueFullCubeViewIndependent} probe instead — the same substitution
 * {@link LandingResolver#classify} makes, and for the same reason.
 */
public final class LandingHitValidationPolicy {
    ...
    public static double shiftedCenterDy(
        ...
        boolean statefulObjectTargetUse = ownerFamily == LandingResolver.Family.OBJECT
                && (ownerState.contains(Properties.OPEN)
                || ownerState.contains(Properties.POWERED))
        ...
        if (heldFamily == LandingResolver.Family.USE_CREATED_FULL_CUBE_CONTACT) {
            boolean supportedOwner = ownerState.getBlock() instanceof SlabBlock
                    || ownerState.getBlock() instanceof BlockEntityProvider
                    || SlabSupport.isOpaqueFullCubeViewIndependent(ownerState);
        ...
        // No face branch is intentional: the landing resolver owns these families on all six faces.

        // Preserve the existing deep-owner envelope exactly; the redirect changes only which
    ...
    public static Direction pointedDripstoneSideContinuationDirection(
        ...
        if (ownerPos == null
                ...
                || !ownerState.contains(PointedDripstoneBlock.VERTICAL_DIRECTION)
                || !insideTranslatedCell(ownerPos, ownerDy, hitPos)) {
            return null;
        }
        return ownerState.get(PointedDripstoneBlock.VERTICAL_DIRECTION);
    }
    ...
    private static boolean isPointedDripstoneOwnerFacingHit(BlockState state, Direction hitFace) {
        return state.getBlock() instanceof PointedDripstoneBlock
                ...
                && state.get(PointedDripstoneBlock.VERTICAL_DIRECTION) == hitFace;
    }
    ...
    private static String directNoItemUseMethodName() {
        try {
            ...
            for (Method method : AbstractBlock.class.getDeclaredMethods()) {
    ...
    private static boolean declaresDirectNoItemUseOverride(Class<?> blockClass) {
        if (DIRECT_NO_ITEM_USE_METHOD_NAME == null
                || blockClass == null
                || !AbstractBlock.class.isAssignableFrom(blockClass)) {
            return false;
        }
        try {
    ...
    private static boolean insideTranslatedSlabShape(
        ...
        if (!ownerState.contains(SlabBlock.TYPE)) {
            return false;
        }
        SlabType type = ownerState.get(SlabBlock.TYPE);
    ...
    private static boolean insideTranslatedCell(BlockPos ownerPos, double ownerDy, Vec3d hitPos) {
        return hitPos.x >= ownerPos.getX() - EPSILON
                && hitPos.x <= ownerPos.getX() + 1.0d + EPSILON
                && hitPos.y >= ownerPos.getY() + ownerDy - EPSILON
