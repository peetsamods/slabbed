...
Fragments exist (PistonBaseBlock in the NO-SUPER capture list; moved=true clears markers+store at the source, now at the chunk write seam `LevelChunkOccupantChangeAnchorMixin`). Consequence, corrected: a frozen lowered block pushed by a piston LOST its stored height, and because the read is store-only with no live fallback the destination resolved to a flat 0.0 for the rest of that block's life — a permanent reset, not a visible pop. RESOLVED (maintainer ruling, 2026-09-06): the destination-cell outcome is now specified and fixed — each moved block's destination receives the source's height bit-for-bit, destroyed blocks receive nothing, the vacated cells clear, and an extending head inherits the base's height in shape as well as in picture.
