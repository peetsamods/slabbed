...
**Status (updated post-C2 review):** C0 LANDED e883abfa (RED law matrix + probes); C1 LANDED c2fe16ef (frozen default ON; TEST 18 live-confirmed); C2 IMPLEMENTED — resolver core + slab/full-block families, awaiting the TEST 19 live gate. C2's disclosed deviations: post-hoc resolver at place-RETURN (not remap steering), recognition/authoring pins kept as frozen-OFF compat (killing them deferred), occlusion refusal deferred (no refusal field yet), client predicted-rollback deferred (block-correction hook needed; interim self-heal on next chunk sync). Reviewer nits carried to C3: replaceable-cell owner misattribution (tall-grass side-click stores the neighbor's depth — fix with C3's steering-side capture), gate slabbed$ownerVisibleDy in the validation mixin on FROZEN_DY_ENABLED or document, add a collision-side A-1 correctness test (dy-triad completeness), fold the beta35 validation lane into the generalized one (two-lane preemption is honestly documented but is still the §1.4 disease).
...
1. **UP face (floor-seated families):** `landingDy = visibleTop(owner) − targetCell.getY()` … minus 0 because the placed block's natural bottom is at its cell floor. For `targetCell = owner.above()` over a full-block owner this is exactly `ownerVisibleDy`; over a bottom-slab owner it is `ownerVisibleDy − 0.5`. This is the generalization of the one already-correct lane in the codebase — the fence formula `supportDy + supportTopOffset − 1.0` at `SlabSupport.java:501-503`. **Caveat verified this change (critic gap #6): that exemplar's `supportDy` input is itself depth-capped** — `beta35FenceWallVisibleSupportDy` (`SlabSupport.java:508-568`) and `floorTorchBottomSlabSupportDy` (`SlabSupport.java:2952-3011`) have the closed codomain {NaN, −1.0, −0.5, 0.0}. The formula shape is right; the input must become the stored/live owner dy.
   - **TOP-owner + UP-click precedence (A-5, normative):** when the clicked owner is a lowered **TOP-type slab** (or DOUBLE) and the face is UP, rule 1 governs and the placed block lands **flush on the owner's visible top** (`landingDy = ownerVisibleDy`). Rule 4 (same-cell DOUBLE merge) applies **only** to a **BOTTOM-type** slab owner clicked UP; a TOP-type owner never merges on an UP-click — the placement goes to the cell above and seats flush. This is exactly the 6-row dy-split geometry (`slab_on_-1.0_TOP_owner`), and it is pinned by `LandingRuleLawTest.slabOnMinus1TopSlabOwnerLandsFlush`, which DEFINES the precedence.
2. **DOWN face (any family) and hanging families regardless of face:** the attachment plane is `visibleBottom(support above)`; `landingDy = visibleBottom(support) − (targetCell.getY() + 1)`. For a full-block support this is `supportDy`; for a lowered TOP slab it is `supportDy + 0.5` — exactly the compensation `ceilingHungDecorationDy` already computes (`SlabSupport.java:2431-2433, :2456`). Attachment direction is thereby first-class: the same arithmetic, applied to the underside plane.
   - **Deep DOWN-face scope limit (A-5, normative):** the deep DOWN-face resolution (e.g. clicking the visible underside of a −1.0 owner → `owner.below(2)` at dy 0.0, §1.3.5 / critic B1) is valid **only for free-standing families** (full cubes, slabs, floor-seated objects). For **attachment-requiring families** (hanging lantern, chain, hanging sign, wall-mounted variants) the resolved grid cell may have no valid attachment support — the cell above `owner.below(2)` is air — so vanilla `canSurvive` refuses the placement (LAW §4 vanilla-mechanic exclusion). The resolver does not force these into a free-standing seat; it defers to `canSurvive`, and a refusal is a legitimate PASS, not a landing bug.
...
5. **Cell choice & occlusion refusal:** the target cell is the cell whose family-natural span, shifted by `landingDy`, does not intersect any existing **visible** body. Generalizes the not-air refusal at `SlabSupport.java:1216-1223` to visible-body occlusion using the existing classifier (`SlabEnsembleCoherence.isOccludedOccupancy`, consumed at `BlockItemPlacementIntentMixin.java:874`). On refusal: cancel with PASS — never fall through to a flat vanilla placement. This is what closes critic B1's down-face/into-body hole: clicking the visible underside of a −1.0 owner resolves to `owner.below(2)` at dy 0.0 (pure arithmetic: `visibleBottom − (cellY+1) = 0`); clicking into a grid-air cell occupied by a deep owner's protruding body refuses.
   - **AIMLESS-overlap policy (A-5, normative):** aimed (non-frozen-OFF) routes cannot mint overlapping visible bodies, but pistons/dispensers/frozen-OFF worlds can. When the player then aims at a cell whose visible span is shared by more than one overlapping body, the resolver does **not** arbitrate geometry — it takes **first-hit-wins by the offset raycast's own ray order** (the nearest visible body the honest raycast attributes the click to, `SlabbedOffsetRaycast`). The resolver refuses (PASS) **only when the TARGET cell itself is occluded** (its family-natural span shifted by `landingDy` intersects an existing visible body), never merely because some *other* overlapping body is nearby. Occlusion refusal is thus a property of the resolved target cell, not of the neighbourhood.
...
|---|---|---|---|---|
| 1 | **Vanilla slabs — UP-click on owner top** (stone/smooth_stone/oak slab) | −1.0 owner → lands 0.0 or −0.5: owner-top marker authored after capture (`BlockItemPlacementIntentMixin.java:1594-1603` vs `BlockOnPlacedAnchorMixin.java:37`); deep-rest excludes exactly −1.0 (`SlabSupport.java:2551`); TOP-slab/hopper owners invisible to `isCompoundTopHit` (`:86-94`); oak refused by stone pins | TOP-SEAT, any owner shape (full block, TOP/DOUBLE/BOTTOM slab, EntityBlock), any depth, any slab family | Kill layer-1/2 pins (§1.4); A7 re-spec (§5). DISCLOSED (C2): the legacy `-Dslabbed.offsetRaycast=false` rollback config loses deep-torch targeting as of C2 — the slab torch-comfort OVERLAY (the legacy retargeter's DDA entry point) was removed, its rescue partner being already dead under the shipping config; the retargeter itself formally dies in C7 (D11) |
...
| 7 | **Doors / tall plants / small dripleaf / pitcher crop** | Double hole: `DoorBlock.setPlacedBy` never calls super → no capture (stored NaN; `BlockOnPlacedAnchorMixin.java:24` injects `Block.setPlacedBy` only); AND landing lanes 0.0 on non-slab owners (`floorTorchBottomSlabSupportDy:2955` isBottomSlab gate; `shouldOffset:978` early return). Evidence (A-2 corrected): the ledger's counted door-GAP session is **TEST 13** (`LIVE_LEDGER.md:17` — "doors 12× landed 0.0 vs 2× −0.5"); TEST 17's per-session summary counts it as **1 `birch_door` GAP** among 4 (`LIVE_LEDGER.md:21`). The "5×" figure is a row-level count of `birch_door` 0.0-on-−0.5 action rows in the TEST-17 `mismatches.tsv` (ledger-uncounted, row-level not summary) | TOP-SEAT (LOWER/foot cell), **pair cell stored too**: UPPER := LOWER's dy | Capture at place-RETURN (§4); pair enumeration via `DOUBLE_BLOCK_HALF` |
| 8 | **Beds** | `BedBlock` calls super FIRST, HEAD placed after via `setBlockAndUpdate` → FOOT captured on a half-formed bed, HEAD never captured; landing via depth-blind presence literals (bed −0.5 on exactly-−1.0 stone — a counted TEST-17 GAP, `LIVE_LEDGER.md:21` "flower_pot + bed at −0.5 on EXACTLY −1.0 supports", row-level 2×; bed −1.0 on marked −1.0 slab where flush is −1.5 — unflagged sibling seam) | TOP-SEAT; both cells stored (`BED_PART` pair) | Pre-implementation probe: recorder row for HEAD stored dy (critic A4) |
...
| 13 | **Stairs** | Hard-flush list `SlabSupport.java:2871` | **Maintainer policy row (D8)** — recommend TOP-SEAT for consistency; if refused, stays an explicit FLUSH-list entry | |
...
**Client mirror:** the store syncs via chunk attachment (`syncWith(...all())`, `SlabAnchorAttachment.java:257`; client lookup `SlabAnchorClientSync.java:92-105`). The in-flight window between place and sync is the observed `LIVE_PLACEMENT_SIDE_DY_SPLIT` (server 0.0 / client −0.5). **Evidence provenance (amendment A-2, corrected C0):** the authoritative per-session count is `LIVE_LEDGER.md:21` — TEST 17 recorded **0 dy-splits, 0 expected-dy mismatches** (the recorder's per-session `summary.md` field `placementSideDySplitRows`, the ledger's headline STAYS metric). The **6 `LIVE_PLACEMENT_SIDE_DY_SPLIT` rows** cited by the slab-landing dossier (`GOES-LANE-DOSSIERS-2026-07-10.md`, CASE (b) VARIANT 2 — 5 `oak_slab[type=top]` owners plus the 1 hopper owner) are **row-level marker entries from the cumulative `mismatches.tsv`, not the per-session summary count** — they are the placement-instant server/client predict split (a distinct column from the ensemble dy-split-per-action figure the ledger headline tracks), which is exactly why the two numbers disagree without either being wrong. **This 6-row set is not yet pinned to a verified session file and must be before it baselines R2's live-gate metric (A-2 open item — §9.5).** Fix: the resolver runs identically on the client's `place` call, writing a **client-predicted dy entry** (generalizing the one existing predicted writer, `updateClientPredictedPersistentLoweredSlabCarrier`, `BlockItemPlacementIntentMixin.java:1608-1613`) that the synced value replaces.
...
**9.5 Remaining pre-implementation probes (C0):** MossyCarpet/snow-stacking census on one canonical jar (A2/D1) — **RESOLVED, see §9.5.1**; bed-HEAD stored-dy recorder row (A4) — **now a gametest**, `LandingRuleLawTest.bedBothCellsStoredAtExactlyMinus1` logs the HEAD cell's stored dy explicitly; TS `ModOnTopBlocksRegistry` runtime contents for carpets (row 5 TS gate — still open, runtime probe); confirm the two 07-07/08 dy-split rows' F1-gate provenance (slab-landing open question — does not change the mechanism).

**9.5.1 MossyCarpet / snow-stacking `setPlacedBy` census (critic A2) — RESOLVED (C0, 2026-07-10).** Run via `javap` against **the one canonical mapped jar the build actually compiles against**: `.gradle/loom-cache/minecraftMaven/net/minecraft/minecraft-common-043a8b3edf/26.2/minecraft-common-043a8b3edf-26.2.jar` (identified as `fabric.gameJarPath` in `.gradle/loom-cache/launch.cfg:7`; the client half is `minecraft-clientOnly-043a8b3edf-26.2.jar`). Findings:

- **`MossyCarpetBlock` OVERRIDES `setPlacedBy` and does NOT call super** (bytecode confirmed: the method body calls `Level.isClientSide`, `createTopperWithSideChance`, then conditionally `Level.setBlock(pos.above(), topper, …)` — no `invokespecial Block.setPlacedBy`). It is therefore a genuine setPlacedBy-override **bypasser** (same class as `DoorBlock`) — the setPlacedBy-HEAD capture (`BlockOnPlacedAnchorMixin.java:37`) never fires for it, confirming §3's route-completeness claim that place-RETURN is required to cover it. Note: the "topper" it may set at `pos.above()` is a **non-item `setBlock`** → stays AIMLESS/uncaptured/flush by design (§2 row 16), which is correct.
- **`SnowLayerBlock` does NOT override `setPlacedBy`** (it overrides only `canBeReplaced`). It inherits `Block.setPlacedBy`, so the HEAD capture fires normally, and **snow-layer stacking is a same-cell `canBeReplaced` merge** (the DOUBLE-merge class, §1.3.4 / family row 3) — NOT a capture-bypass hole. The C5 concern for snow layers is the perf fast-path (A-3) and the AIM-KEYED thin-layer seat, not a missed capture point.
- **`CarpetBlock` / `WoolCarpetBlock` do NOT override `setPlacedBy`** (inherited) — plain carpets capture faithfully; only the mossy variant bypasses. `snowLayerStackingPreservesStore` (§6.1 C5) can therefore be built on the merge-re-store mechanism, not a new capture point.