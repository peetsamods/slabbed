...
| S11 | Port the best-known fix, not the last shipped jar — and don't trust a known false-green gate. | Reproducing an inferior architecture across versions because the superior fix lived only on an unmerged candidate branch, then "validating by absence" with a test that is a documented false green. (26.2 carried the 3075-line post-hoc targeting retargeter for months while the clean `SlabbedOffsetRaycast` overhaul sat unported; the slab-held lowered-upper/-1.0-compound WYSIWYG bug was never caught because it "didn't come up" live and the client targeting gametest passes while live is wrong.) | When a fix exists only on a candidate branch, record it as explicit port debt in `PORTING_MAP.md` and port THAT, not the shipped lineage. For targeting, prove the non-false-green geometry invariant (a block with `getYOffset != 0` must be resolved by the offset raycast to its offset `getShape` outline) AND run a WYSIWYG matrix (lane × held-item) live — never close targeting on the client gametest alone (it is a known false green). | `slabbed-targeting-overhaul-never-ported-trap` (memory), `docs/binder/26-2-offset-raycast-overhaul-candidate-20260621.md`, `docs/binder/26-2-slab-held-target-owner-identity-20260621.md` |
