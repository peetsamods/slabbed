...
These rows came from Julia's `Proofs 26.2.pdf` review. They started as exact live/manual symptoms that could not be closed from headless `runGameTest` alone. Julia later found one release-blocking false-green in the chained speleothem family, and then a broader slab-held targeting / WYSIWYG rollback-baseline red after the rejected `27a03f...` candidate was removed. The current source tree carries headless fixes for some rows, but the active `SLABBED-MC 26.2` profile is intentionally back on rollback jar SHA `ba19ebe6f1f8a75ce3de123bcd7179fbf5b062233c01cc4a49de865684e182db` until the rollback-baseline live blocker is classified. Pre-release hygiene must not resume from memory or old staged-candidate assumptions.
...
`mods/_codex-backups/`. The earlier raw-owner-override suspicion was valid for that candidate, but the rejected source
experiment has now been removed from active source. The current blocker is the rollback-baseline live red itself:
prove where visible model, selectable outline, and final `Minecraft.pick` / placement target disagree before another
candidate jar is staged.
...
| P26-5 | Source fixed headlessly, but not active in the current rollback profile | `runGameTest-sulfur-spike-green.log`, `p26-4-5-dripstone-sulfur-live.png`, Julia live pass, `tmp/runGameTest-speleothem-ceiling-bridged-chain-red.log`, `tmp/runGameTest-speleothem-ceiling-bridged-chain-green.log`, `tmp/runGameTest-speleothem-direct-top-red.log`, `tmp/runGameTest-speleothem-direct-top-green.log` | after the rollback-baseline targeting/WYSIWYG blocker is classified, rebuild/stage a deliberate candidate and live-retest pointed dripstone + sulfur spike chained directly under top slabs |
...
| P26-9 | Open on rollback baseline; rejected source experiment removed | `tmp/runClientGameTest-slab-held-owner-guard.log`, `tmp/runGameTest-slab-held-owner-guard.log`, `tmp/build-slab-held-owner-guard.log`, `tmp/compile-source-aligned-targeting-rollback.log`, live screenshots `/var/folders/qd/dqqdc7fd5pndvbpcrbqkshdh0000gn/T/codex-clipboard-94402963-32f4-4c0e-bd53-c75a5c95c8d4.png`, `/var/folders/qd/dqqdc7fd5pndvbpcrbqkshdh0000gn/T/codex-clipboard-3d11eb71-13d7-4614-8b5e-d21f959e2c1b.png`, `/var/folders/qd/dqqdc7fd5pndvbpcrbqkshdh0000gn/T/codex-clipboard-92a82608-8f52-4fbb-b70e-0eb9af6a8a47.png`, `/var/folders/qd/dqqdc7fd5pndvbpcrbqkshdh0000gn/T/codex-clipboard-7ce390e9-7e00-4627-97a2-f907c6be0910.png`, `/var/folders/qd/dqqdc7fd5pndvbpcrbqkshdh0000gn/T/codex-clipboard-5c7d39f7-b21f-4863-b282-ac0b89645bfb.png`, `/var/folders/qd/dqqdc7fd5pndvbpcrbqkshdh0000gn/T/codex-clipboard-cfc1ed40-34a6-459c-9e8d-12d191ab5d25.png`, rollback jar SHA `ba19ebe6f1f8a75ce3de123bcd7179fbf5b062233c01cc4a49de865684e182db` | on the rollback baseline, prove where visible model, selectable outline, and final `Minecraft.pick` / placement target disagree before another client candidate |
