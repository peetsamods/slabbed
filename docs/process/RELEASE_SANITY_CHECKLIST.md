...
These rows came from Julia's `Proofs 26.2.pdf` review. They started as exact live/manual symptoms that could not be closed from headless `runGameTest` alone. Julia later found one release-blocking false-green in the chained speleothem family, and then a broader slab-held targeting / WYSIWYG red on the rollback baseline after the rejected `27a03f...` candidate was removed.

**RESOLVED + RELEASED 2026-06-21 (`0.4.2-beta.1+26.2`):** the slab-held targeting / WYSIWYG blocker is fixed — not by another patch to the legacy retargeter, but by porting the offset-aware nearest-hit raycast (`SlabbedOffsetRaycast` + `LocalPlayerPickOffsetRaycastMixin`); opposite-side compound placement is fixed too. Julia LIVE-CONFIRMED both. The `SLABBED-MC 26.2` profile is on the released jar SHA `5140fb50…` (byte-identical to the verified targeting+opposite-fix build). **Pre-release hygiene MAY proceed.** Remaining items are the documented KNOWN ISSUES (on-top placement flicker; deep-stack mixed-offset gaps = the deferred slab-combining feature) — see HANDOFF current-state + CHANGELOG. This paragraph supersedes the earlier hold gate (hygiene is no longer paused).
...
| P26-9 | Slab-held targeting owner identity | holding a slab must not select/outline a different owner than the block under the crosshair; slab vs non-slab held-item may differ only in legal placement, not visible owner truth | live red + GT/recorder if harnessable + VIS |
...
P26-9 has a false-green warning. Focused client coverage for the slab-held owner-identity law passed on the ordinary
lowered bottom-slab lane and the compound-visible upper slab lane path, but Julia live-rejected the staged
`27a03f...` jar because targeting became worse in general: slab-held `Oak Slab`, `dy=-0.500 LOWERED`, `side=up`,
`half=UPPER`, `sro=ANCHORED` displaced the selector from the crosshair-visible owner and then fell through to
`target: none`. The `SLABBED-MC 26.2` profile jar has been rolled back to SHA-256
`ba19ebe6f1f8a75ce3de123bcd7179fbf5b062233c01cc4a49de865684e182db`; the rejected `27a03f...` jar is preserved under
`mods/_codex-backups/`. **RESOLVED 2026-06-21:** rather than patch the legacy retargeter again, the targeting
ARCHITECTURE was replaced with the offset-aware nearest-hit raycast (`SlabbedOffsetRaycast` +
`LocalPlayerPickOffsetRaycastMixin`); crosshair/outline/HUD/pick/placement now agree by construction. Julia
LIVE-CONFIRMED. Shipped in `0.4.2-beta.1+26.2` (released jar SHA `5140fb50…`). Row CLOSED; the remaining
on-top-placement flicker is a documented KNOWN ISSUE, not a release blocker.
...
| P26-5 | CLOSED — fix shipped in released `0.4.2-beta.1+26.2` (jar `5140fb50…`) | `runGameTest-sulfur-spike-green.log`, `p26-4-5-dripstone-sulfur-live.png`, Julia live pass, `tmp/runGameTest-speleothem-ceiling-bridged-chain-red.log`, `tmp/runGameTest-speleothem-ceiling-bridged-chain-green.log`, `tmp/runGameTest-speleothem-direct-top-red.log`, `tmp/runGameTest-speleothem-direct-top-green.log` | shipped + staged; live pointed-dripstone/sulfur-spike-under-top-slab covered by Julia's live pass |
...
| P26-9 | CLOSED — resolved by the offset-raycast overhaul, shipped `0.4.2-beta.1+26.2`, Julia live-confirmed (on-top flicker = documented KNOWN ISSUE, not a blocker) | `tmp/runClientGameTest-slab-held-owner-guard.log`, `tmp/runGameTest-slab-held-owner-guard.log`, `tmp/build-slab-held-owner-guard.log`, `tmp/compile-source-aligned-targeting-rollback.log`, live screenshots `/var/folders/qd/dqqdc7fd5pndvbpcrbqkshdh0000gn/T/codex-clipboard-94402963-32f4-4c0e-bd53-c75a5c95c8d4.png`, `/var/folders/qd/dqqdc7fd5pndvbpcrbqkshdh0000gn/T/codex-clipboard-3d11eb71-13d7-4614-8b5e-d21f959e2c1b.png`, `/var/folders/qd/dqqdc7fd5pndvbpcrbqkshdh0000gn/T/codex-clipboard-92a82608-8f52-4fbb-b70e-0eb9af6a8a47.png`, `/var/folders/qd/dqqdc7fd5pndvbpcrbqkshdh0000gn/T/codex-clipboard-7ce390e9-7e00-4627-97a2-f907c6be0910.png`, `/var/folders/qd/dqqdc7fd5pndvbpcrbqkshdh0000gn/T/codex-clipboard-5c7d39f7-b21f-4863-b282-ac0b89645bfb.png`, `/var/folders/qd/dqqdc7fd5pndvbpcrbqkshdh0000gn/T/codex-clipboard-cfc1ed40-34a6-459c-9e8d-12d191ab5d25.png`, rollback jar SHA `ba19ebe6f1f8a75ce3de123bcd7179fbf5b062233c01cc4a49de865684e182db` | RESOLVED: targeting overhaul (`SlabbedOffsetRaycast`) shipped in `0.4.2-beta.1+26.2`, jar `5140fb50…` |
...
| B6 | place slab on visible UP face of a lowered side-placed TOP slab | new slab chooses above cell, `dy=-0.500`, no underside merge | GT + DY + VIS |
| B7 | compare slab-held vs non-slab-held crosshair owner on lowered stacked slabs | same visible owner under crosshair; no slab-held retarget steal | live red + targeting proof |
