...
P26-5 chained downward speleothem follow-up: Julia's pre-release spin found the ceiling-bridged iron-chain stack could still merge. The new red proof `tmp/runGameTest-speleothem-ceiling-bridged-chain-red.log` failed because the lower pointed-dripstone and sulfur-spike segments inherited `dy=+0.5` while the upper segment stayed flush under the visible chain bridge. `Slabbed2612RestingDyTest.downwardPointedDripstoneColumnUnderCeilingBridgedChainStaysFlush` and `Slabbed2612RestingDyTest.downwardSulfurSpikeColumnUnderCeilingBridgedChainStaysFlush` now guard that exact case. Green proof: `tmp/runGameTest-speleothem-ceiling-bridged-chain-green.log` passed 128/128. Live retest is still required before marking this row release-closed again.
...
| P26-5 | Headless-fixed after Julia found a chained speleothem false-green; staged for live retest | `runGameTest-sulfur-spike-green.log`, `p26-4-5-dripstone-sulfur-live.png`, Julia live pass, `tmp/runGameTest-speleothem-ceiling-bridged-chain-red.log`, `tmp/runGameTest-speleothem-ceiling-bridged-chain-green.log` | restart `SLABBED-MC 26.2` and live-retest pointed dripstone + sulfur spike chained under ceiling-bridged iron chain |
