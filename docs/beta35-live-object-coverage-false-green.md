...
|---|---|---|---|---|
| `minecraft:birch_fence` | Not in `isBeta35FenceWallVariantContactObject`; render `dy` forced to `0.0f`; contact-dy path returns NaN. | `sideSlabRetargetFired=true` on slab; `warning=BOTTOM_SLAB_UNEXPECTED_DY`; held in 269 retargeting rows. | `EXACT_BLOCK_ALLOWLIST_GAP`, `VARIANT_FAMILY_COVERAGE_GAP`, `FENCE_RENDER_VARIANT_GAP`, `PROOF_HARNESS_UNDER_SCOPE` | **RED proof exists** (`-Dslabbed.beta35BirchFenceVariantRed=true`, `JULIA_BETA35_BIRCH_FENCE_VARIANT_SUMMARY outcome=RED failureLayer=VARIANT_FAMILY_COVERAGE_GAP`). See `docs/beta35-birch-fence-variant-red.md`. Next: expand allowlist to include `birch_fence` narrowly. |
...
1. `birch_fence` lowered bottom-slab contact + render-quad RED — **DONE** (`-Dslabbed.beta35BirchFenceVariantRed=true`; `JULIA_BETA35_BIRCH_FENCE_VARIANT_SUMMARY outcome=RED`; see `docs/beta35-birch-fence-variant-red.md`). Next: expand `isBeta35FenceWallVariantContactObject` to add `BIRCH_FENCE` narrowly.
