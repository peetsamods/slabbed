...
## The cull gate differs per renderer — and Maintainer's env is Sodium
...
**Consequence:** the live profile (`Slabbed 1.21.1`) runs **Sodium**, which provides the FRAPI renderer itself and **bypasses Indigo entirely**. So the window Maintainer sees is produced by *Sodium's* occlusion. A vanilla `Block.shouldDrawSide` mixin would NOT affect Sodium (it reimplements), and the Indigo `BlockRenderInfo` mixin is **dead code under Sodium**. Only a **Sodium `BlockOcclusionCache.shouldDrawSide` mixin** fixes it in her environment. (Indigo/vanilla mixins still help users who run *without* Sodium.)
