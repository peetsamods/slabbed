...
## Follow-up RED proof (2026-05-12)

Recommendation A (proof harness fix) was implemented as a new gated proof: `-Dslabbed.beta35FenceModelRenderRed=true` in `SlabbedLabLoweredSidePlacementLiveReproClientGameTest`. It calls `OffsetBlockStateModel.resetModelDyOwnerTrace(objectPos)` plus `mc.worldRenderer.scheduleBlockRenders(...)` and snapshots `OffsetBlockStateModel.snapshotModelDyOwnerTrace()` after `waitForChunksRender`. On HEAD `a576fa1` the four allowlisted variants `minecraft:oak_fence`, `minecraft:spruce_fence`, `minecraft:nether_brick_fence`, and `minecraft:cobblestone_wall` all classify as `MODEL_RENDER_GAP` with `objectDy=-1.500000`, `expectedModelDy=-1.500000`, `emitCalls=6`, `appliedCalls=0`, `totalAppliedDy=0.000000`, and `renderDyApplied=no`. Summary: `JULIA_BETA35_FENCE_MODEL_RENDER_SUMMARY outcome=RED failureLayer=MODEL_RENDER_GAP`. The false-green mechanism described above is confirmed and reproducible. The production render fix (Recommendation B) is the next slice. See `docs/beta35-fence-model-render-red.md`.
