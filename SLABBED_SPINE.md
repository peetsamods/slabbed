...
Automated live-shape goblin harness is now gated-only behind `-Dslabbed.beta4LiveShapeGoblin=true`. The harness is not part of the default client gametest route; `SlabbedLabLoweredSidePlacementLiveReproClientGameTest` invokes it only when the opt-in flag is present. `SlabbedLabBeta4LiveShapeGoblinClientGameTest` builds Julia's canonical live structure before testing: two bottom support slabs, a two-block full-block bridge, two slabs on top of that bridge, and one upper full block on top of one of those slabs. It emits `[JULIA_BETA4_LIVE_GOBLIN_STRUCTURE_GREEN]` only after proving the structure, then automates Julia's lower-half side A/B, upper-half side A/B, top-face, support-break, and hitbox/owner checks with a final `[JULIA_BETA4_LIVE_GOBLIN_SUMMARY]`. Lower-half side A/B are GREEN through the narrow server hit-validity bridge for held-slab legal compound remaps: both author `stone_slab[type=bottom]` at `dy=-0.5` with no lower-side wrong owner or ghost. Upper-half side A/B and support-break remain GREEN. Top-face is now GREEN by clean reject/preserve: held-slab `UP` clicks on the compound upper full block return `Pass[]`, author no top/skipped slab, keep the source at compound `dy=-1.0`, and emit `[JULIA_BETA4_LIVE_GOBLIN_TOP_FACE_GHOST_GREEN]`. Latest goblin summary: `structure=GREEN lowerA=GREEN lowerB=GREEN upperA=GREEN upperB=GREEN topFace=GREEN supportBreak=GREEN hitbox=RED ghost=false jump=false wrongOwner=true releaseBlockers=none`. Hitbox/wrongOwner remain diagnostic from existing upper-side owner/miss observations, not release-blocking in this summary. No beta4 `dy=-1.0` slab lane or `dy<-1.0` lane is legalized. Release is ready for Julia manual live retest; do not treat automation alone as release approval unless Julia explicitly waives the manual gate.
