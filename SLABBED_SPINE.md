...
Manual-live delayed final trace after `4e6dae9` / `save/beta4-manual-live-placement-trace`: Julia's manual run captured real click-path REDs where immediate client results can look successful before server reconciliation, including lower-side client success, repeat/upper-side `Pass[]`, and a top-face success candidate that may correspond to the ghost/floating report. The trace remains diagnostics-only and now adds delayed snapshots at client ticks 1, 5, 20, and 40 with `[JULIA_BETA4_MANUAL_LIVE_DELAYED_FINAL]` and `[JULIA_BETA4_MANUAL_LIVE_DELAYED_SUMMARY]`. Immediate client delta is not durable proof; release remains **blocked** until Julia reruns the manual trace and the delayed final states are interpreted before any implementation slice.
