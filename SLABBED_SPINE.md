...
Automated live-shape goblin harness is now gated-only behind `-Dslabbed.beta4LiveShapeGoblin=true`. The harness is not part of the default client gametest route; `SlabbedLabLoweredSidePlacementLiveReproClientGameTest` invokes it only when the opt-in flag is present. `SlabbedLabBeta4LiveShapeGoblinClientGameTest` builds Julia's canonical live structure before testing: two bottom support slabs, a two-block full-block bridge, two slabs on top of that bridge, and one upper full block on top of one of those slabs. It emits `[JULIA_BETA4_LIVE_GOBLIN_STRUCTURE_GREEN]` only after proving the structure, then automates Julia's lower-half side A/B, upper-half side A/B, top-face, support-break, and hitbox/owner checks with a final `[JULIA_BETA4_LIVE_GOBLIN_SUMMARY]`. Latest goblin summary: `lowerA=RED lowerB=RED upperA=GREEN upperB=GREEN topFace=RED supportBreak=GREEN hitbox=RED ghost=true jump=false wrongOwner=true releaseBlockers=lowerSide,topFace`. This is harness/proof only; release remains **blocked**.
