...
## Beta 3.5 fence/wall stack contact fix (2026-05-12)

- Operating base: HEAD `aa66efd` / `save/beta35-fence-wall-owner-server-hit` on `integrate/phase19-into-side-slab-top-support`. Scope is the remaining wall/fence stack-contact gap plus residual triad/owner classification. No release audit run, no release tag moved, no all-item expansion, and no server hit tolerance rewrite.
- Julia live trace after `aa66efd` showed server validation was green (`SERVER_HIT_TOO_FAR=0`, `SERVER_SHIFTED_HIT_GREEN=21`) but one concrete contact bucket remained: `stone_brick_wall` over lowered `stone_brick_wall` support with `objectDy=-0.500000`, `supportDy=-1.000000`, `supportVisibleTopY=-55.000000`, `objectModelBottomY=-54.500000`, and `contactGap=0.500000`.
- Production fix: `SlabSupport.beta35FenceWallVisibleSupportDy(...)` now treats lowered `FenceBlock` / `WallBlock` support states as legal visible-support candidates for the already-scoped fence/wall contact path. The top object dy is derived from the support visible top for legal wall/fence stack rows. `ServerInteractBlockHitToleranceMixin` was not modified.
- Focused proof: `-Dslabbed.beta35FenceWallStackContact=true -Dslabbed.beta35FenceWallLiveInspect=true`; markers `JULIA_BETA35_FENCE_WALL_STACK_CONTACT_ROW`, `JULIA_BETA35_FENCE_WALL_STACK_CONTACT_SUMMARY`, `JULIA_BETA35_FENCE_WALL_STACK_CONTACT_GREEN`, and `JULIA_BETA35_FENCE_WALL_RESIDUAL_JANK_SUMMARY`.
- Result: GREEN. `JULIA_BETA35_FENCE_WALL_STACK_CONTACT_SUMMARY outcome=GREEN rows=4 green=4 contactGap=0 triadMismatch=0 ownerGap=0 other=0 wallOnWall=GREEN fenceEquivalents=GREEN previousFailureLayer=FENCE_WALL_STACK_CONTACT_DY_MISSING failureLayer=NONE serverHitTooFarRows=0 serverHitToleranceChanged=false releaseAudit=NOT_RUN releaseTagMoved=false`.
- Reproduced wall-on-wall after fix: `objectDy=-1.000000`, `supportDy=-1.000000`, `supportVisibleTopY=-55.000000`, `objectModelBottomY=-55.000000`, `contactGap=0.000000`, `triadCoLocated=yes`, `finalDecision=object-shape-owner-preserve`. Fence-on-fence, wall-on-fence, and fence-on-wall fixture stack rows are also GREEN; no new placement legality is claimed.
- Residual jank classification: connected-wall triad rows are classified as `TRACE_FALSE_POSITIVE` / vanilla connected-wall shape limit unless a future focused proof shows a Slabbed-lowered triad bug. Residual owner gaps are classified as out-of-scope held-item/category rows or a next focused RED target if post-fix live capture finds in-scope fence/wall/anvil visible-owner failures.
- Validation passed: `compileJava compileGametestJava`, focused stack contact proof, owner/server-hit proof, fence/wall contact-hitbox proof, live-hitbox owner proof, live-hitbox gate proof, fence/wall family proof, fence-gate family proof, common-object matrix, floor_torch plain-bottom contact, candle contact, flower_pot contact/survival, default `runClientGameTest`, runClient live-inspect startup smoke, and `git diff --check`.
- **Beta 3.5 release remains BLOCKED pending Julia live acceptance.** Evidence folder: `tmp/beta35-fence-wall-stack-jank-fix-aa66efd/`. See `docs/beta35-fence-wall-stack-contact-fix.md`.
