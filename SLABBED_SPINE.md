...
Automated live-shape goblin harness is now gated-only behind `-Dslabbed.beta4LiveShapeGoblin=true`. The harness is not part of the default client gametest route; `SlabbedLabLoweredSidePlacementLiveReproClientGameTest` invokes it only when the opt-in flag is present. `SlabbedLabBeta4LiveShapeGoblinClientGameTest` builds Julia's canonical live structure before testing: two bottom support slabs, a two-block full-block bridge, two slabs on top of that bridge, and one upper full block on top of one of those slabs. It emits `[JULIA_BETA4_LIVE_GOBLIN_STRUCTURE_GREEN]` only after proving the structure, then automates Julia's lower-half side A/B, upper-half side A/B, top-face, support-break, and hitbox/owner checks with a final `[JULIA_BETA4_LIVE_GOBLIN_SUMMARY]`. Lower-half side A/B are now GREEN through the narrow server hit-validity bridge for held-slab legal compound remaps: both author `stone_slab[type=bottom]` at `dy=-0.5` with no lower-side wrong owner or ghost. Upper-half side A/B and support-break remain GREEN. Top-face remains RED, and hitbox/wrongOwner remain RED because of the remaining top/upper owner cases, not the lower-side route. Latest goblin summary: `lowerA=GREEN lowerB=GREEN upperA=GREEN upperB=GREEN topFace=RED supportBreak=GREEN hitbox=RED ghost=true jump=false wrongOwner=true releaseBlockers=topFace`. No beta4 `dy=-1.0` slab lane or `dy<-1.0` lane is legalized. Release remains **blocked** by topFace.
