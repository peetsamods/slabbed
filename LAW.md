...
- A hung decoration (item frame, glow frame, painting) is an entity with no block store, so it
  ...
  not the decoration. "Follows the support" is the same violation as a neighbor edit moving a
  placed block under Law 1.
- Any lane that re-derives a height from neighbours on read — on this line, the legacy geometry
  walk that answers for a cell carrying no stored placement fact — is outside this law. It exists
  only for worlds older than the store and is not a second definition of correct behaviour.
