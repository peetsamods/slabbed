...
- **Definition of Ready includes a LAW-PREFLIGHT answer** for any package touching `src/main`: *"does
  this let a neighbor move a placed block? (n — or y + Maintainer's sign-off)."* If the honest answer is y,
  mark the package `Status: blocked-needs-decision` and surface it to Maintainer — do not route.
...
- **Commit after each successful pass** (reviewer `commit-ready: yes` + the `tools/hooks` guardrails
  green). One reviewed unit → one commit; never batch. Behavior changes then stop for Maintainer's live A/B
  (S-5 / `docs/process/LIVE_LEDGER.md`).
- Maintainer is the human owner. Surface real decisions to her; never guess them.
